...
<section class="vote-hero">
  <div class="wrap vote-hero__grid">
    <div><p class="eyebrow">The league has spoken</p><h1>League<br>Votes</h1><p>Manager ballots, weekly predictions, and the public results that turn twelve opinions into one league conversation.</p><div class="vote-hero__actions"><a class="button button--gold" href="#active-votes">See active votes</a><a class="text-link" href="{{ '/votes/power-rankings/' | relative_url }}">Power Rankings <span aria-hidden="true">→</span></a></div></div>
    <aside class="vote-principles"><span>Built for a static league site</span><strong>Free. Private.<br>Commissioner reviewed.</strong><ul><li>Google Forms collects ballots</li><li>Only sanitized results are published</li><li>Yahoo remains read-only</li></ul></aside>
  </div>
</section>
...
<section class="shell-content vote-section" id="active-votes" aria-labelledby="active-votes-heading">
  <div class="section-heading"><div><p class="eyebrow">Your voice, on the record</p><h2 id="active-votes-heading">Active Votes</h2><p>Rule proposals, awards, and other commissioner-approved ballots appear here only when a real voting window is configured.</p></div><span class="vote-status-badge">{{ public_votes.active_polls.size }} open</span></div>
  {% if public_votes.active_polls.size > 0 %}
    <div class="poll-grid">
      {% for poll in public_votes.active_polls %}<article class="poll-card"><div class="poll-card__meta"><span>{{ poll.type | replace: '_', ' ' }}</span>{% if poll.closes_at %}<time datetime="{{ poll.closes_at }}">Closes {{ poll.closes_at | date: '%b %-d, %-I:%M %p' }}</time>{% endif %}</div><h3>{{ poll.title }}</h3><p>{{ poll.description }}</p>{% if poll.results.size > 0 %}<div class="poll-results">{% for option in poll.results %}<div><span><strong>{{ option.label }}</strong><small>{{ option.vote_count }} votes · {{ option.percentage | times: 100 | round: 1 }}%</small></span><i style="--result: {{ option.percentage | times: 100 }}%"></i></div>{% endfor %}</div>{% endif %}{% if poll.submission_url %}<a class="button button--gold" href="{{ poll.submission_url }}" target="_blank" rel="noopener noreferrer">Open ballot</a>{% endif %}</article>{% endfor %}
    </div>
  {% else %}
...
<section class="content-section vote-section vote-archive" id="vote-archive" aria-labelledby="vote-archive-heading"><div class="wrap">
  ...
  {% if public_votes.archived_polls.size > 0 %}<div class="poll-grid">{% for poll in public_votes.archived_polls %}<article class="poll-card"><div class="poll-card__meta"><span>{{ poll.season }}</span><span>{{ poll.ballots_counted }} ballots</span></div><h3>{{ poll.title }}</h3><p>{{ poll.description }}</p></article>{% endfor %}</div>{% else %}<div class="vote-empty"><span aria-hidden="true">—</span><div><h3>No completed votes yet</h3><p>Only real commissioner-approved results will enter this archive.</p></div></div>{% endif %}
</div></section>
