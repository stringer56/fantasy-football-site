...
try:
    from .voting_common import file_fingerprint
except ImportError:
    from voting_common import file_fingerprint
...
def build_status(
    *, root: Path = ROOT, season: int = 2026, week: int | None = None,
    ...
    return {
        ...
        "yahoo": {
            "status": "current" if manifest.get("status") == "ready" and manifest.get("season") == season else "unavailable",
